...
class PetFriends:
    ...
    def get_api_key(self, email: str, password: str) -> json:
        """Метод делет запрос к API сервера и возвращает татус запроса и результат в формате JSON и
        уникальным ключем пользователя, найденого по уникальным email и паролем"""
    ...
    def get_list_of_pets(self, auth_key: json, filter: str = "") -> json:
        """Метод делет запрос к API сервера и возвращает татус запроса и результат в формате JSON
        со списком найденых питомцев, совпадающих с фильтром. На данным момент фильтр может иметь
        любо пустое значение - получить список всех питомцев, либо 'my_pets' - получить список
    ...
    # Первая часть практического задания : доделать нереализованные методы.
